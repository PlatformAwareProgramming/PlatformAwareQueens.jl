...
function gpu_queens_tree_explorer!(size_v::Val{size}, cutoff_depth_v::Val{cutoff_depth}, number_of_subproblems, 
                                   ...
                                   indexes_d) where {size, cutoff_depth}
	@inbounds begin
...
function queens_gpu_caller(size, cutoff_depth, number_of_subproblems, starting_point, subproblems)
    ...
    @cuda threads=__BLOCK_SIZE_ blocks=num_blocks gpu_queens_tree_explorer!(Val(size), Val(cutoff_depth), number_of_subproblems, subpermutation_d, controls_d, tree_size_d, number_of_solutions_d, indexes_d)
